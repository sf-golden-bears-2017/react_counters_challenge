...
If you start needing to pass arguments to `increment` and `decrement` functions, you'll want to keep in mind that `<MyComponent onClick={this.increment(someArgument, someOtherArgument)} />` is doing that second thing — calling the function and passing its return value, instead of passing the function itself like we want.
